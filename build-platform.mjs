...
['results','배포와 그 이후',`<h3>설치 단계에서 막히지 않도록 EV 서명을 붙였습니다</h3><p>서명되지 않은 설치 파일은 Windows SmartScreen이 경고를 띄웁니다. 설치 직전에 사용자를 잃는 지점이라 DUNS 번호를 발급받아 조직 확인을 거친 뒤 EV 코드 서명 인증서를 받아 빌드에 연결했습니다. 클라우드 서명을 자동 모드로 걸어 빌드할 때마다 별도 입력 없이 서명과 타임스탬프가 붙게 했습니다.</p><h3>새 버전을 사용자가 직접 받지 않게 했습니다</h3><p>처음에는 버전을 올릴 때마다 사용자가 설치 파일을 다시 내려받아야 했습니다. 앱이 켜지면 새 버전을 확인해 백그라운드로 내려받고 완료되면 앱 안에서 안내하도록 바꿔 사용자는 재시작만 하면 되게 했습니다.</p><p>배포 경로는 소스 저장소와 분리했습니다. 릴리스 파일만 올리는 공개 저장소를 따로 두어 앱에 접근 토큰을 넣지 않고도 업데이트를 받을 수 있게 했습니다.</p><p>배포 이후에는 실행 환경에서 올라오는 문의를 받아 원인을 좁히고 수정 버전을 올렸습니다.</p><p>버전을 올릴 때마다 같은 항목을 다시 점검해 이전에 고친 문제가 재발하지 않는지 확인했습니다. 코드를 작성한 시점이 아니라 실제 환경에서 동작을 확인한 시점을 작업의 끝으로 두었습니다.</p><a class="external" href="https://lolpago.com/" target="_blank" rel="noopener noreferrer">직접 개발한 LOLPAGO 웹사이트 ↗</a>`],
...
{id:'tournament',name:'대회 운영 업무 자동화 시스템',category:'루모스 / 외부 고객 의뢰',periodLabel:'개발 기간',period:'2026.04 (이후 운영과 유지보수)',role:'2인 개발. 업무 흐름 설계와 기능 구현, 발주자 요구사항 확인과 진행 보고 담당',stack:'PHP / MySQL',metric:'22',unit:'하나의 흐름으로 연결한 운영 기능',desc:'단계마다 끊기던 대회 운영을 하나의 데이터 흐름으로 묶었습니다.',summary:'화면을 먼저 만들지 않았습니다. <br class="opt">신청부터 상장 발행까지 이어지는 업무를 하나의 데이터 흐름으로 정의한 뒤 <br class="opt">기능 사이의 관계를 설계했습니다.',sections:[
...
['results','결과와 해석',`<div class="result"><strong>51/90 → 80/90</strong><p>동일한 30개 질문을 개선 전후 각각 3회씩 실행했을 때의 자동 통과 응답 수입니다.</p></div><p>검색 회귀는 1,870문항 골든셋으로 따로 돌려 전 문항 통과를 확인했습니다. 검증 질문은 50문항에서 시작해 1,870문항까지 늘렸습니다. 검색 회귀와 답변 평가는 서로 다른 검사이므로 결과를 나눠 기록했습니다. 고친 오답은 기준 문항으로 남겨 갱신할 때마다 재발하는지 확인했습니다.</p><p class="note">정해진 질문 표본과 보유 자료에 대한 결과이며 전체 서비스 정확도나 사용자 만족도를 뜻하지 않습니다.</p>`],
